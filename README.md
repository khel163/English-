...
<head>
    ...
    <title>Village Committee Donation & Expense Register dhanpuri</title>
    <style>
...
<body>
    ...
    <section class="form-section">
        ...
        <form id="donationForm">
            <input type="text" id="name" placeholder="doner नाम" required />
    ...
    <footer>
        <p>© 2025 Village Committee | All Rights Reserved</p>
    </footer>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function updateSummary() {
                ...
                totalDonationDisplay.textContent = `Total Donation: ₹${totalDonation.toFixed(2)}`;
            ...
            window.editItem = function(type, index) {
                ...
                const amount = prompt('Amount:', data.amount);
            ...
            document.getElementById('printButton').addEventListener('click', () => {
                let printContent = `<h2>Donation Details</h2>` + document.getElementById('donationTable').outerHTML;
                printContent += `<h2>Expense Details</h2>` + document.getElementById('expenseTable').outerHTML;
                printContent += `<h2>Summary</h2><div>${totalDonationDisplay.textContent}</div><div>${totalExpenseDisplay.textContent}</div><div>${balanceDisplay.textContent}</div>`;
            ...
            document.getElementById('downloadPdf').addEventListener('click', () => {
                ...
                doc.text("Village Committee Donation & Expense Report", 14, 15);
                ...
                doc.text(`Total Donation: ₹${totalDonation.toFixed(2)}`, 14, finalY);
                doc.text(`Total Expense: ₹${totalExpense.toFixed(2)}`, 14, finalY + 8);
                doc.text(`Balance: ₹${balance.toFixed(2)}`, 14, finalY + 16);

                doc.save('village-committee-report.pdf');
            });
            ...
            document.getElementById('downloadExcel').addEventListener('click', () => {
                ...
                XLSX.utils.book_append_sheet(wb, donationSheet, "Donations");
                XLSX.utils.book_append_sheet(wb, expenseSheet, "Expenses");

                XLSX.writeFile(wb, "village-committee-report.xlsx");
            });
